...
async function getVideoMetadata(path) {
  return new Promise((resolve, reject) => {
    ffmpeg.ffprobe(path, function (err, metadata) {
      ...
      resolve(metadata.streams.find((s) => s.codec_type === 'video'));
    });
  });
}
...
module.exports = {
  ...
  init(config) {
    ...
    const upload = async (file, customParams = {}) => {
      const path = file.folderPath !== "/" ? file.folderPath : ""; // with trailing slash if present, otherwise empty
      ...
      try {
        ...
        if (file.mime.startsWith('video/')) {
          const metadata = await getVideoMetadata(file.url);
